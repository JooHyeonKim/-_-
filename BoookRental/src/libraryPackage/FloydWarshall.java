...
public class FloydWarshall //플로이드 워샬을 통한 최단거리 검색입니다.
{
	...
	{			
		...
		Library.textPane.append("최단거리 : " + String.format("%.4f", Collections.min(rent.sum)) +"km");//최단 거리 출력
		Library.textPane.append("\n");
		...
		for(int i=0; i < rent.path.length; i++)//최단 이동경로 출력
		{
			for(int b=0; b<rent.rentbook.size(); b++)//그 도서관에 있는 책들을 모두 빌렸다고 추가합니다.
			{
				...
				{	
					Library.textPane.append("'" + rent.rentbook.get(b) + "' 책은 " + rent.path[minIndex][i] + "도서관에서 빌리실 수 있습니다.\n");
