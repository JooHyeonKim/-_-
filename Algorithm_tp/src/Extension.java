...
public class Extension {
	...
	private static boolean available() throws IOException {
		...
		while (line != null) {
			...
			if (rent_splited[1].equals(userId) && rent_splited[2].equals(book_num) && rent_splited[6].equals(lib_num)) {
	...
	private static boolean isReserved(String rentNum) throws IOException {
		...
		while (temp != null) {
			...
			splited = temp.split("\t");
			if (splited[2].equals(book_num) && splited[6].equals(lib_num) && !splited[0].equals(rentNum)) return true; //reserved
		}
